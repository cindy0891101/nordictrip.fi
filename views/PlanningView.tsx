...
const PlanningView: React.FC<PlanningViewProps> = ({ members }) => {
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        <div className="flex items-center gap-3 mb-2 px-1">
          ...
          <div className="flex items-center gap-3">
            <h2 className="font-bold text-sage text-2xl tracking-tight">{member?.name} 的行李箱</h2>
          </div>
        </div>
              ...
              return (
                <div key={cat.id} className="space-y-3">
                  <div 
                    ...
                    className="flex items-center justify-between py-3 px-5 bg-[#DDD3C9]/40 border border-[#D2C2B2]/50 rounded-full cursor-pointer active:opacity-80 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-lg bg-sage/10 text-sage flex items-center justify-center">
                        <i className={`fa-solid ${cat.icon} text-xs`}></i>
                      </div>
                      <span className="text-sm font-bold text-sage tracking-tight">{cat.label}</span>
                      <span className="bg-white/60 text-[9px] font-bold text-earth-dark/70 px-2.5 py-0.5 rounded-full border border-paper">
                      ...
                      {catItems.length > 0 ? catItems.map(item => (
                        <div key={item.id} className="bg-white py-2.5 px-5 rounded-[1.25rem] border border-paper/30 flex items-center justify-between shadow-sm group/item animate-in fade-in slide-in-from-top-1 duration-300">
                          <div onClick={() => toggleItem(item.id)} className="flex items-center gap-4 flex-grow cursor-pointer">
                            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${item.completed ? 'bg-sage border-sage' : 'border-paper/60'}`}>
                          ...
                          <button onClick={() => deleteItem(item.id)} className="text-earth-dark/30 hover:text-stamp transition-colors w-8 h-8 flex items-center justify-center">
                            <i className="fa-solid fa-trash-can text-xs"></i>
                          </button>
                        </div>
                      )) : (
                        <div className="py-4 text-center">
                          <p className="text-[10px] font-bold text-earth-dark/30 italic uppercase tracking-[0.2em]">尚無項目</p>
                        </div>
                      )}
                    </div>
                  )}
          ...
          <div className="space-y-2">
            {items.length > 0 ? items.map(item => (
              ...
              <div className="py-24 text-center">
                 <p className="text-xs font-bold text-earth-dark/30 italic uppercase tracking-[0.2em]">尚無採買項目</p>
              </div>
            )}
          </div>
        )}
        ...
        <button 
          ...
          className="w-full h-12 border-2 border-dashed border-sage/20 rounded-full bg-white/40 flex items-center justify-center gap-2 text-sage font-bold active:scale-95 transition-all text-sm shadow-md hover:bg-white hover:border-sage mt-6"
        >
          <i className="fa-solid fa-plus-circle"></i> 新增項目
        </button>
      </div>
    );
  };
  ...
  return (
    <div className="px-4 overflow-x-hidden animate-in fade-in duration-500 -mt-4"
     style={{ paddingBottom: '6.5rem' }}>
      ...
      {activeTab === 'todo' && (
        <div className="space-y-4">
          <NordicButton onClick={() => setShowAddTodo(true)} className="w-full h-14 bg-stamp border-none text-xs uppercase tracking-[0.2em] font-bold shadow-xl">
            <i className="fa-solid fa-plus mr-2"></i> 新增團隊待辦
          </NordicButton>
          <div className="space-y-3">
              ...
              <div key={item.id} className="bg-white py-3 px-5 rounded-[1.5rem] border border-paper/30 flex items-center justify-between shadow-md animate-in fade-in slide-in-from-left-2 duration-300">
                <div onClick={() => {
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[9px] font-bold text-earth-dark/50 uppercase tracking-[0.15em] mt-1">
                      {item.assignedTo === 'ALL' ? '全體成員' : (members.find(m => m.id === item.assignedTo)?.name || '未知')}
                    </span>
                  </div>
                </div>
                <button onClick={() => deleteTodo(item.id)} className="w-10 h-10 flex items-center justify-center text-earth-dark/20 hover:text-stamp transition-all">
                  <i className="fa-solid fa-trash-can text-sm"></i>
                </button>
              </div>
            )) : <div className="py-24 text-center border-2 border-dashed border-paper/30 rounded-[3rem] bg-white/10">
              <p className="text-xs text-earth-dark/20 font-bold uppercase tracking-[0.2em] italic">目前尚無待辦事項</p>
            </div>}
          </div>
        </div>
      )}
      ...
      {(activeTab === 'packing' || activeTab === 'shopping') && (
        selectedMemberId ? renderChecklist() : renderMemberSelect()
      )}
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
          <NordicCard className="p-6 bg-white/80 backdrop-blur-md rounded-[2.5rem] border-paper/20">
            <div className="flex items-center justify-between mb-4 px-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <select 
                  ...
                  onChange={(e) => setCurrentAuthorId(e.target.value)}
                  className="bg-white border-2 border-paper/30 rounded-full text-[10px] px-3 py-1 outline-none font-bold text-sage shadow-sm"
                >
            ...
            <textarea 
              ...
              placeholder="分享關於旅程的資訊、連結 or 心情..."
              className="w-full min-h-[120px] p-5 bg-white border-2 border-paper/30 rounded-[2rem] text-sm font-bold text-sage outline-none resize-none shadow-inner"
            />
            ...
            {infoImage && (
              <div className="mt-4 relative inline-block">
                ...
                <button 
                  ...
                  className="absolute -top-2 -right-2 w-7 h-7 bg-stamp text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white"
                >
          ...
          <div className="space-y-6">
            {travelInfos.length > 0 ? travelInfos.map(info => {
              ...
              return (
                <div key={info.id} className="relative group animate-in slide-in-from-bottom-2 fade-in duration-300">
                  <div className="absolute -top-4 -right-2 z-10">
                    <img src={author?.avatar || 'https://picsum.photos/seed/unknown/100/100'} className="w-12 h-12 rounded-full border-4 border-white shadow-xl object-cover" alt="" />
                  </div>
                  ...
                  <NordicCard className="p-7 bg-white rounded-[2.5rem] border-paper/20">
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-[10px] font-bold text-earth-dark/40 uppercase tracking-widest">
                        {new Date(info.createdAt).toLocaleString('zh-TW', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </span>
                      <button onClick={() => deleteInfo(info.id)} className="opacity-0 group-hover:opacity-40 hover:!opacity-100 text-stamp transition-opacity p-1">
                        <i className="fa-solid fa-trash-can text-sm"></i>
                      </button>
                    </div>
                    ...
                    {info.imageUrl && (
                      <div className="mt-5 rounded-[2rem] overflow-hidden border border-paper/20 shadow-lg">
                        <img src={info.imageUrl} className="w-full max-h-80 object-cover" alt="Info" />
                      </div>
                    )}
                  </NordicCard>
                </div>
              ...
              <div className="py-24 text-center border-2 border-dashed border-paper/30 rounded-[3rem] bg-white/10">
                <i className="fa-solid fa-comment-dots text-5xl text-earth-dark/5 mb-4"></i>
                <p className="text-xs text-earth-dark/20 font-bold uppercase tracking-[0.2em] italic">目前尚無共享資訊</p>
              </div>
            )}
          </div>
        </div>
      ...
      <Modal isOpen={showAddTodo} onClose={() => setShowAddTodo(false)} title="新增團隊待辦">
        <div className="space-y-6 pb-4">
          <div className="space-y-2">
            ...
            <input 
              type="text" 
              placeholder="例如：訂購機場接送..." 
              value={todoInput.text}
              onChange={(e) => setTodoInput({ ...todoInput, text: e.target.value })}
              className="w-full p-3.5 bg-white border-2 border-paper/30 rounded-[1.75rem] font-bold text-sage outline-none shadow-sm" 
            />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-earth-dark uppercase tracking-widest pl-1">負責人</label>
            <select 
              value={todoInput.assignedTo}
              onChange={(e) => setTodoInput({ ...todoInput, assignedTo: e.target.value })}
              className="w-full p-3.5 bg-white border-2 border-paper/30 rounded-[1.75rem] font-bold text-sage outline-none shadow-sm appearance-none"
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {PACKING_CATS.map(cat => (
                  <button 
                    ...
                    className={`py-2.5 px-3 rounded-2xl text-[11px] font-bold transition-all border-2 flex items-center gap-3 ${newItem.category === cat.id ? 'bg-sage text-white border-sage shadow-lg' : 'bg-white text-earth-dark/60 border-paper/20'}`}
                  >
                    <i className={`fa-solid ${cat.icon} text-xs`}></i>
                    {cat.label}
                  </button>
